...
| `apl_review_bundle.sh` | Maintainer-review-side; **optional** for agents (the PR preflight no longer requires a bundle) and generated by the maintainer review flow. | Acceptable to keep. Optional nicety: a portable Python entrypoint so a Windows agent can self-generate a bundle if it wants one. |
