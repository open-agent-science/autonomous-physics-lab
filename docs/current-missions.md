...
proposed OQMD result remains in open PR #1634 and is not repository evidence
until merge.
...
3. replay CHARA `RESULT-0031` only when an executor clears the identity-
   independence check against the now-recorded publisher, preserving its
...
| [Textbook Formula Audit](./campaigns/textbook-formula-audit.md) | Public verifier lane with four scoped AGENT_VALIDATED results and AGENT_PUBLISHED INCONCLUSIVE CHARA `RESULT-0031`; publisher provenance is repaired | Use an identity-independent Gate B replay when a qualifying executor is available; keep HD 284163 separate |
