...
| --- | --- | --- |
| [Nuclear Mass Surface](./campaigns/nuclear-mass-surface.md) | Flagship validation surface with baseline residuals, sandbox scouts, frozen predictions, no-leakage contract, and reveal-readiness blockers; local-curvature no-leakage is falsified under `TASK-0394`, and the first residual-free high-error cluster audit is `INCONCLUSIVE` under `TASK-0449` | local-curvature negative/preflight packaging, registry/reveal-readiness reporting, neutron-rich boundary transfer, magic-distance interaction controls, negative-result preservation |
