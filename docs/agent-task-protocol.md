...
gate remains the fallback and the source of truth for local diagnosis. Create
the repository label before first use. A ready transition means the draft-lane
checks and review gate were green enough to request review; for code or
workflow changes, the ready transition can start the broader post-ready PR
checks, and branch protection still requires those checks to pass before merge.
