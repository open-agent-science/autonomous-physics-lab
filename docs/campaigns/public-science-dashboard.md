...
| FRB / Radio Transients | Can a time-truncated exposure surface support a sealed repeater-propensity prediction without label leakage? | The Catalog-1 interval exposure pair passed checksum/schema gates, `TASK-0963` built a 479-row pre-T exposure feature surface, `TASK-0964` froze the exposure-only model surface, `PRED-0001` was registered as a 479-source point-score/rank prediction, and the anchor capsule is sealed by GitHub Release tag `pred-frb-pret-repeater-propensity-20260710`. | Complete the reduced rights-bounded anchor capsule; activate reveal-source work only on a concrete official release signal. | A citable checksum anchor or later trigger-based reveal-source decision; no current success verdict. |
| [Quantum Size Effects](./quantum-size-effects.md) | What new source or contract would add information beyond the completed ZnSe/InP transfer failure? | Calibration-derived rows remain excluded; Almeida 2023 yielded six direct InP rows; ZnSe/Toufanian rows are frozen as limited factual extracts; AGENT_VALIDATED `RESULT-0029` records the strict no-refit transfer miss as INCONCLUSIVE control memory; TASK-0988 found a SOURCE_LIMITED_MONITOR CdSe lead. | Package the Kim-2020 CdSe source artifact value-blind; stop if it remains source-limited. | A source-artifact readiness verdict, not a broad model claim. |
...
| [Textbook Formula Audit](./textbook-formula-audit.md) | Can CHARA or a prospective Gaia DR4 route provide a cleaner external stellar transfer surface? | Four results are AGENT_VALIDATED; CHARA has thirteen alias candidates after two known DEBCat overlaps; Gaia DR4 has a prospective contract but no values or targets have been read. | Complete CHARA aliases and one consolidated Gaia DR4 freeze-readiness packet. | A source/readiness GO/HOLD/STOP verdict, not stellar values, metrics, or universal formula wording. |
| [Materials Property Residuals](./materials-property-residuals.md) | Can APL turn open, published materials databases into reusable benchmark datasets and conservative residual maps? | `MD-0001` landed as a first reusable-dataset candidate; `MD-0002` is `AGENT_VALIDATED` as `RESULT-0021` and externally citable; TASK-0993 selected OQMD v1.8 as a possible independent second-source surface. | OQMD metadata-only rights, semantics, retrieval-identity, and MD-0002 overlap preflight. | A source-readiness verdict, not a download, dataset, metric, or material recommendation. |
...
| Lattice-QCD Aggregated Consistency | Can one convention-stable observable support an auditable dependency graph that adds value beyond FLAG? | Source policy exists, but no real observable row, average, metric, result, or claim has been admitted. | Run one observable/dependency/covariance incubator; no loader or full campaign before GO. | `GO_LATTICE_DEPENDENCY_AUDIT`, a covariance hold, or `STOP_DUPLICATES_FLAG`; no Lattice-QCD physics claim. |
