...
| [Textbook Formula Audit](./textbook-formula-audit.md) | Can APL audit famous formulas by source, range, assumptions, and OOD failure maps? | Stefan-Boltzmann has a Gate-B-validated exact-reference result; Stellar M-L, FIRAS/Wien, and high-mass transfer are AGENT_VALIDATED in narrow scope; TASK-0989 pinned a value-blind CHARA package and excluded two known DEBCat overlaps. | Identifier-complete alias and whole-system de-dup audit for thirteen CHARA candidates. | A source-readiness verdict, not stellar values, metrics, or universal formula wording. |
