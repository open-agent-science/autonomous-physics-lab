...
| [Quantum Size Effects](./quantum-size-effects.md) | Can the pinned Kim-2020 CdSe source support deterministic factual-row extraction without reopening model search? | AGENT_VALIDATED `RESULT-0029` preserves the strict ZnSe/InP no-refit miss as INCONCLUSIVE control memory; Kim-2020 is now pinned as a CC BY 4.0 source artifact with figure, morphology, and uncertainty routes recorded but no values extracted. | Freeze text-versus-digitized row classes, optical-axis semantics, uncertainty, and anisotropy before any extraction. | An extraction-contract readiness or source-limited verdict, not a CdSe size-law result. |
...
| [Textbook Formula Audit](./textbook-formula-audit.md) | Can CHARA or a prospective Gaia DR4 route provide a cleaner external stellar transfer surface? | Four results are AGENT_VALIDATED; thirteen CHARA systems are now alias-clean against DEBCat, but seven share Hyades membership; Gaia DR4 component masses still need a valid same-component bolometric-luminosity route. | Curate CHARA per-component rows and freeze cluster dependence value-blind; keep Gaia at GO/HOLD/STOP readiness. | A row-readiness/dependence verdict, not stellar metrics or universal formula wording. |
...
| [Dimensional Analysis Validator](./dimensional-analysis-validator.md) | Can label-blind dimensional inference clear an honestly fresh benchmark without policy-equivalence credit? | Historical 50/74 surfaces remain reproducible calibration memory; TASK-1038 separated inference from expected labels and made exact agreement the primary v2 metric, so the old surface is development-only. | Freeze a fresh role-disjoint 80-120 item benchmark without running the validator. | A no-score benchmark-readiness verdict first; metrics and any Gate C decision come later. |
