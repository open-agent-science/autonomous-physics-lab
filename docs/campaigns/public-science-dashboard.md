...
| [Exoplanet Mass-Radius](./exoplanet-mass-radius.md) | After stronger null-baseline controls and host-context missingness checks, is any CK17-style residual slice still worth another residual pilot? | Compact-radius was the strongest earlier matched-control diagnostic, but `TASK-0483` found nearest-radius nulls match or beat CK17-style residuals in the highlighted true-mass slices, and landed `TASK-0481` found no benchmark-usable compact-radius host-context axis under the current coarse-bin floor. | `TASK-0515` control-aware go/no-go synthesis before any further residual pilot | A benchmark panel that either declares a narrow conditional follow-up or demotes the compact-radius host-context story to negative/control memory. |
...
- Mass-quartile localization and host-context coarse bins are currently
  underpowered, so the next useful test is a control-aware go/no-go rather
  than stronger planet-physics interpretation.
