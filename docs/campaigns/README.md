...
| help build the next direct-measurement dataset | [Quantum Size Effects](./quantum-size-effects.md) | Almeida 2023 produced six direct InP rows and a source-scoped sandbox baseline; `RESULT-0029` packages the ZnSe no-refit transfer as an AGENT_VALIDATED inconclusive/control result, and TASK-0988 found only a monitor-only CdSe source lead. |
...
| work on reusable benchmark datasets | [Materials Property Residuals](./materials-property-residuals.md) | It has `MD-0001`, `MD-0002`, AGENT_VALIDATED `RESULT-0021`, and a citable MD-0002 release; `TASK-1026` is the metadata-only OQMD second-source gate before any new acquisition. |
| work on source-pinned thermophysical data | [Thermophysical Property Residuals](./thermophysical-property-residuals.md) | It has AGENT_VALIDATED ThermoML `Tb` / Joback `RESULT-0026` on a bounded 40-row fixture and AGENT_VALIDATED failed-family `RESULT-0028`; the current blocker is expansion/source access, not broader chemistry claims. |
...
| [Quantum Size Effects](./quantum-size-effects.md) | Almeida 2023 direct InP rows, source-scoped sandbox baseline, controls/holdout summary, frozen factual ZnSe rows, AGENT_VALIDATED `RESULT-0029` preserving the strict no-refit transfer miss, failed effective-mass transfer memory, and a SOURCE_LIMITED_MONITOR CdSe lead. | No repeat transfer on the same ZnSe/InP surface; pursue a new source-artifact task only if the CdSe route is approved. |
...
| [Materials Property Residuals](./materials-property-residuals.md) | `MD-0001`, `MD-0002`, AGENT_VALIDATED `RESULT-0021`, scope-control memory, transfer-negative replay, and a citable Zenodo MD-0002 dataset DOI. | OQMD v1.8 source-manifest, semantics, retrieval, and overlap preflight; no dump download, rows, or metrics. |
| [Textbook Formula Audit](./textbook-formula-audit.md) | AGENT_VALIDATED exact-reference `RESULT-0019`, Stellar M-L `RESULT-0022`, FIRAS/Wien `RESULT-0023`, and high-mass transfer `RESULT-0024`; CHARA has a value-blind source package with two known DEBCat overlaps excluded. | Identifier-complete CHARA alias/de-dup audit; no stellar values, metrics, or universal formula claim. |
| [Thermophysical Property Residuals](./thermophysical-property-residuals.md) | AGENT_VALIDATED ThermoML `Tb` `RESULT-0026` on a 40-row family-stratified fixture, with a frozen Joback baseline, rights boundary, and AGENT_VALIDATED esters/lactones negative/control `RESULT-0028`. | Decide the source-access contract; do not replay `RESULT-0028`, broaden to other properties, or rerun Joback as a new result. |
...
| [Textbook Formula Audit](./textbook-formula-audit.md) | Scaffold landed; Stefan-Boltzmann, Stellar M-L, FIRAS/Wien, and Stellar high-mass transfer are agent-validated within narrow scopes; the CHARA second-source package is value-blind and still alias-gated. | Complete identifier aliases and whole-system DEBCat de-dup; no model-derived Gaia mass truth rows or universal formula claims. |
...
| [Thermophysical Property Residuals](./thermophysical-property-residuals.md) | Newly active source-pinned benchmark lane with AGENT_VALIDATED ThermoML Tb RESULT-0026 and AGENT_VALIDATED esters/lactones failed-family RESULT-0028. | Expansion remains source-access/family-count gated; no raw archive vendoring or multi-property broadening. |
