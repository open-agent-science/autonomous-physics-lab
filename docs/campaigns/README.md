...
- **Default near-term science-output sprint:** fixed-relation CHARA transfer,
  OQMD no-peek split and within-source controls, independent replay of
  `RESULT-0030`, the `f_K/f_pi` dependency pilot, and source-provenance
  adjudication for the stopped CdSe route. FRB, Exoplanet, Nuclear, and Atomic
  remain reveal- or trigger-gated.
...
| monitor a stopped direct-measurement route | [Quantum Size Effects](./quantum-size-effects.md) | Almeida 2023 produced six direct InP rows and `RESULT-0029`; both four-row Kim-2020 axes are underpowered and no independent CdSe route cleared the frozen gates. |
...
| work on reusable benchmark datasets | [Materials Property Residuals](./materials-property-residuals.md) | It has `MD-0001`, `MD-0002`, AGENT_VALIDATED `RESULT-0021`, a citable release, and AGENT_PUBLISHED `INVALID` OQMD `RESULT-0032` awaiting independent replay. |
| inspect source-pinned thermophysical evidence | [Thermophysical Property Residuals](./thermophysical-property-residuals.md) | It has AGENT_VALIDATED `RESULT-0026` and failed-family `RESULT-0028`; exact-80 is stopped, while an at-most-74-row extraction contract is source-ready. |
...
| test a new source-gated campaign | [Fresh Physics Data Axes](./fresh-physics-data-axes.md) | Lattice `f_K/f_pi` has two partial-hold dependency audits and one live value-admissibility gate; no central values or full campaign are authorized. |
...
| --- | --- | --- |
| [Nuclear Mass Surface](./nuclear-mass-surface.md) | `NMD-0003`, negative factory memory, exact-replayed `RESULT-0025`, failed uncertainty calibration, frozen point predictions, and a predeclared four-entry point-only reveal interpretation. | Event-trigger monitoring; no source access or scoring before a qualifying manifest. |
...
| [Quantum Size Effects](./quantum-size-effects.md) | Almeida direct InP rows, AGENT_VALIDATED `RESULT-0029`, Kim-2020 `UNCERTAINTY_BLOCKED` memory, two underpowered four-row axes, and no independent CdSe route. | Monitor for a genuinely new qualifying source; no score, pooled axes, or repeat scout. |
| [Atomic-Clock Residuals](./atomic-clock-residuals.md) | Beloy and Nemitz `171Yb/87Sr` rows, source-limited consistency memory, blocked alternatives, and a ratified `KEEP_MONITOR_ONLY` reopen-trigger ledger. | Wait for a qualifying source event; no constants-drift or metric rerun. |
| [Materials Property Residuals](./materials-property-residuals.md) | `MD-0001`, `MD-0002`, AGENT_VALIDATED `RESULT-0021`, a citable DOI, and AGENT_PUBLISHED `INVALID` OQMD `RESULT-0032`. | Identity-independent replay only; preserve the failed structured-null gate. |
| [Textbook Formula Audit](./textbook-formula-audit.md) | Four narrow AGENT_VALIDATED results plus AGENT_PUBLISHED INCONCLUSIVE CHARA `RESULT-0031`; HD 284163 and current Gaia DR4 are source-semantics stops. | Identity-independent CHARA replay and identity-only DEBCat leakage-control freeze. |
| [Thermophysical Property Residuals](./thermophysical-property-residuals.md) | AGENT_VALIDATED `RESULT-0026` and failed-family `RESULT-0028`; exact-80 is stopped and an at-most-74-row contract is source-ready. | Extract a complete fixture under article-cap and information-floor gates; score only later. |
