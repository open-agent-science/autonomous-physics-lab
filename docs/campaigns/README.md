...
| --- | --- | --- |
| [Nuclear Mass Surface](./nuclear-mass-surface.md) | `NMD-0003` frozen stratified gate, negative factory memory, exact-replayed `RESULT-0025`, failed uncertainty calibration, and a four-entry point-only reveal family with predeclared interpretation. | Event-triggered source discipline; no interval claim, early scoring, broad F2, shell, local-curvature, Wigner, or ad hoc reveal interpretation. |
...
| [Textbook Formula Audit](./textbook-formula-audit.md) | Four narrow results are AGENT_VALIDATED; CHARA `RESULT-0031` is AGENT_PUBLISHED INCONCLUSIVE, while HD 284163 and the pinned Gaia DR4 draft are stopped on component-luminosity semantics. | Perform identity-independent replay without rescuing the margin; reopen source routes only on new admissible semantics. |
