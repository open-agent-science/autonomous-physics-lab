...
- **Fresh-data buildout:** Atomic-Clock Residuals and Quantum Size Effects
  ...
  and `RESULT-0029`, an AGENT_PUBLISHED ZnSe/InP no-refit transfer result that
...
- **New thermophysical benchmark lane:** Thermophysical Property Residuals has
  ...
  AGENT_PUBLISHED esters/lactones failed-family negative/control result.
...
| work on source-pinned thermophysical data | [Thermophysical Property Residuals](./thermophysical-property-residuals.md) | It has AGENT_VALIDATED ThermoML `Tb` / Joback `RESULT-0026` on a bounded 40-row fixture and AGENT_PUBLISHED failed-family `RESULT-0028`; the current blocker is expansion/source access, not broader chemistry claims. |
| monitor the exoplanet benchmark surface | [Exoplanet Mass-Radius](./exoplanet-mass-radius.md) | It has pinned snapshots, benchmark diagnostics, target-freeze protocol, reviewer capsule, null-baseline control panel, `NO_NOTIFY` monitor check 3, and AGENT_PUBLISHED `RESULT-0027`; new scoring waits for a source-version trigger. |
...
| [Materials Property Residuals](./materials-property-residuals.md) | `MD-0001`, `MD-0002`, AGENT_VALIDATED `RESULT-0021`, family-holdout scope boundary, descriptor-ablation control memory, transfer-negative replay, and a citable Zenodo MD-0002 dataset DOI. | Record-back and integrity checks are complete; no immediate metric task or archive rebuild without a versioned release reason. |
| [Textbook Formula Audit](./textbook-formula-audit.md) | AGENT_VALIDATED exact-reference `RESULT-0019`, Stellar M-L `RESULT-0022`, FIRAS/Wien `RESULT-0023`, and high-mass transfer `RESULT-0024`; CHARA is paper-level-pin-ready. | Value-blind CHARA source artifact/de-dup package; no universal formula claim. |
| [Thermophysical Property Residuals](./thermophysical-property-residuals.md) | AGENT_VALIDATED ThermoML `Tb` `RESULT-0026` on a 40-row family-stratified fixture, with a frozen Joback baseline, rights boundary, and AGENT_PUBLISHED esters/lactones negative/control `RESULT-0028`. | Replay `RESULT-0028` and decide the source-access contract; do not broaden to other properties or rerun Joback as a new result. |
...
| [Textbook Formula Audit](./textbook-formula-audit.md) | Scaffold landed; Stefan-Boltzmann, Stellar M-L, FIRAS/Wien, and Stellar high-mass transfer are agent-validated within narrow scopes; the second-source Stellar route is SOURCE_LIMITED. | CHARA source-readiness/de-dup preflight; no model-derived Gaia mass truth rows or universal formula claims. |
