...
plus the equivalent macOS private temporary-directory mirrors for sessions
where `/tmp` resolves through the platform's private temp root, and Windows
`C:/tmp/apl-*` mirrors used by the adaptive pytest fast lane.
...
`claude-*` under the declared temp roots are read/write-allowed. An agent that
wants to read something else from those roots still has to ask.

On Windows, `scripts/validate_fast.py` creates a unique
`C:/tmp/apl-pytest-*` directory automatically. Claude Code can use that
allowlisted root without prompting for each session-specific directory. Codex
permissions are controlled by the host app rather than this file; approve the
scoped `python ... scripts/validate_fast.py` command prefix once when the app
requests it.
