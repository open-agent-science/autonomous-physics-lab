...
- `TASK-0136` - Split repository validation and scientific-memory integrity checks (`code_quality_refactor`, priority `medium`, difficulty `medium`, domain `repository_validation`)
...
- `TASK-0254` - Add reusable nuclear factory target-batch library (`dataset_workflow`, priority `medium`, difficulty `medium`, domain `nuclear_physics`)
...
- `TASK-0255` - Add worktree settings sync script and first-action CLAUDE.md instruction (`contributor_experience`, priority `high`, difficulty `low`, domain `contributor_experience`)
- `TASK-0256` - Simplify onboarding prompt task-choice guidance (`agent_workflow`, priority `high`, difficulty `low`, domain `agent_onboarding`)
