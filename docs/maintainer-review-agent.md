...
21. Generated-file policy: the PR does not commit a **new** frequently-changing
    generated/cache file as an agent-facing data source (a "board-like" artifact
    that would change on most task PRs). Committed generated files are allowed
    only as human-facing navigation auto-regenerated by the `Sync Active Board`
    action (`docs/task-views/*.md`) or source-coupled metadata with a CI
    `--check` (`campaigns/catalog.yaml`). Otherwise the view should be exposed
    through an on-demand script. See `docs/generated-file-policy.md` (this is the
    `tasks/ACTIVE.md` / `campaigns/task-index.yaml` mistake).
