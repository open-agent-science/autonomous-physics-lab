...
def _build_report(
    ...
    for fr in search["families"]:
        ...
        if fr["hits"]:
            ...
            if any(h["is_reference"] for h in fr["hits"]):
                lines.append(
                    "Reference-fit rows are shown for calibration only and are excluded from hit counts,"
                    " guardrail checks, and the global verdict."
                )
                lines.append("")
        else:
            lines += ["No hits within 1σ.", ""]

    bz = summary["best_z_score"]
    lines += [
        "## Summary",
        "",
        "| Metric | Value |",
    ...
    if summary["best_formula"]:
        lines.append(f"| {primary_label} | `{summary['best_formula']}` |")
    closest_formula = summary.get("closest_hit_formula")
...
def run_g2_formula_experiment(
    ...
    task_id = str(config.get("task_id", "TASK-0127"))
    ...
    primary_z_text = (
        f"{summary['best_z_score']:.3f}σ" if summary["best_z_score"] is not None else "N/A"
    )
    primary_formula = summary["best_formula"] or "none"
