"""TASK-0202 neutron-rich nuclear sandbox batch.

Sandbox-only batch runner. Evaluates two neutron-rich residual candidates
...
- HYP-PROPOSAL-0033 (quartic asymmetry):
  r_corr = a * I^2 + b * I^4 with I = (N - Z) / A.
- HYP-PROPOSAL-0034 (asymmetric neutron-excess, proton-rich neutral):
  r_corr = a * max(N - Z, 0)^2 / A.

The second candidate is explicitly designed so that its feature is
identically zero on proton-rich rows (N < Z), addressing the one-way
subset trade that HYP-PROPOSAL-0022 and HYP-PROPOSAL-0029 exhibit
between neutron-rich and proton-rich subsets.
...
CANDIDATE_FAMILIES = {
    "quartic_asymmetry": {
        "proposal_id": "HYP-PROPOSAL-0033",
        "title": "Quartic isospin-asymmetry residual",
        "formula": "r_corr = a * I^2 + b * I^4 ; I = (N - Z) / A",
        "feature_names": ("i_sq", "i_quartic"),
        "parameter_count": 2,
    },
    "asymmetric_neutron_excess": {
        "proposal_id": "HYP-PROPOSAL-0034",
        "title": "Asymmetric neutron-excess residual (proton-rich neutral)",
        "formula": "r_corr = a * max(N - Z, 0)^2 / A",
...
REJECTED_PROPOSALS = (
    {
        "proposal_id": "HYP-PROPOSAL-0035",
        "title": "In/Sb chain-specific anchor (post-hoc leakage)",
        "rejection_reason": (
            "A Z in {49, 51} indicator would memorize the In/Sb cluster "
            "identified retrospectively as the worst-case region on the "
            "post-AME2020 primary holdout (AGENT-RUN-0008). Building a "
            "feature around an identified failure cluster is post-hoc "
            "leakage under docs/nuclear-mass-robustness-gate.md."
        ),
    },
    {
        "proposal_id": "HYP-PROPOSAL-0036",
        "title": "Free-power isospin-asymmetry I^p with p fitted",
        "rejection_reason": (
            "Letting the asymmetry exponent be a free fit parameter "
            "introduces a nonlinear knob and exits the linear-additive "
            "correction protocol followed by RESULT-0015 and prior nuclear "
            "sandbox runs. On an 11-row NMD-0002 training surface this is "
            "an unbounded overfit risk and is rejected on complexity grounds."
        ),
    },
    {
        "proposal_id": "HYP-PROPOSAL-0037",
        "title": "Per-shell N=50/82/126 indicator stack",
        "rejection_reason": (
            "A stack of one-hot indicators for individual neutron shell "
            "closures would target the specific shells visible in the "
            "AGENT-RUN-0008 worst-residual list (especially N=82 for the "
            "In/Sb cluster). Rejected as targeted post-hoc leakage; "
            "duplicates the rejection rationale of HYP-PROPOSAL-0031 from "
            "the shell-aware batch."
        ),
    },
)
...
def feature_values(family: str, *, z: int, n: int, a: int) -> tuple[float, ...]:
    if family == "quartic_asymmetry":
        i = float(n - z) / float(a)
        return (i * i, i * i * i * i)
    if family == "asymmetric_neutron_excess":
        excess = max(n - z, 0)
        return (float(excess * excess) / float(a),)
...
def build_metrics() -> dict[str, object]:
    ...
    summary = {
        ...
        "improved_neutron_rich_delta_ge_20_count": sum(
            1 for item in executed_items
            if (
                item["post_ame2020_eval"]["metrics_by_subset"]["neutron_rich_delta_ge_20"]["mae_mev"]
                is not None
                and float(
                    item["post_ame2020_eval"]["metrics_by_subset"]["neutron_rich_delta_ge_20"]["mae_mev"]
                )
                < float(
                    baseline_post_ame2020["metrics_by_subset"]["neutron_rich_delta_ge_20"]["mae_mev"]
                )
            )
        ),
    ...
    return {
        ...
        "formula": "batch: neutron-rich residual candidates (quartic asymmetry, asymmetric neutron-excess)",
        ...
        "feature_definitions": {
            "i_sq": "((N - Z) / A)^2",
            "i_quartic": "((N - Z) / A)^4",
            "neutron_excess_sq_over_a": "max(N - Z, 0)^2 / A (zero on proton-rich rows)",
        },
        ...
        "negative_control_reference": {
            ...
            "rationale": (
                "HYP-PROPOSAL-0022 is the prior overfit/negative-control "
                "quadratic-asymmetry family. The neutron-rich lane is "
                "compared against its retrospective post-AME2020 metrics to "
                "make subset-trade behavior visible by construction."
            ),
        },
        ...
        "limitations": [
            ...
            "HYP-PROPOSAL-0034 is asymmetric by construction (zero on proton-rich rows); this is a design choice, not a discovered asymmetry.",
        ],
    }
