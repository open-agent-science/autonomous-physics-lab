"""TASK-0451 nuclear magic-distance Z×N interaction control lane.
...
SHELL_ADJACENT_DIAGNOSTIC (no registry expansion is authorized
...
AGENT_RUN_ID = "AGENT-RUN-0045"
TASK_ID = "TASK-0451"
HYP_PROPOSAL_PATH = (
    "hypothesis_proposals/nuclear-mass/"
    "HYP-PROPOSAL-0053-magic-distance-interaction.yaml"
)
EXP_PROPOSAL_PATH = (
    "experiment_proposals/nuclear-mass/"
    "EXP-PROPOSAL-0019-nuclear-magic-distance-interaction.yaml"
)
...
MATCHED_DEGREE_RANDOM_SEED = 451
SURVIVAL_MARGIN_MEV = 0.25
LOO_SIGN_FLIP_THRESHOLD = 2

# Custom verdict vocabulary per TASK-0451 specification
CUSTOM_VERDICTS = (
    "SHELL_ADJACENT_DIAGNOSTIC",
    "CONTROL_DOMINATED",
...
CUSTOM_TO_SCHEMA_VERDICT = {
    "SHELL_ADJACENT_DIAGNOSTIC": "REVIEW_NEEDED",
...
def interaction_feature(z: int, n: int) -> float:
    """`exp(-z_dist^2/8) * exp(-n_dist^2/8)`. Closed-form, residual-free."""
    z_dist = magic_distance(z)
    n_dist = magic_distance(n)
    return math.exp(-(z_dist**2) / GAUSSIAN_TWO_SIGMA_SQUARED) * math.exp(
        -(n_dist**2) / GAUSSIAN_TWO_SIGMA_SQUARED
    )
...
def decide_verdict(metrics: dict[str, Any]) -> tuple[str, list[str]]:
    ...
    rationale.append(
        ...
        "SHELL_ADJACENT_DIAGNOSTIC per the custom verdict vocabulary; no "
        "registry expansion is authorized."
    )
    return "SHELL_ADJACENT_DIAGNOSTIC", rationale
...
def build_metrics() -> dict[str, Any]:
    ...
    metrics: dict[str, Any] = {
        ...
        "feature_definition": {
            "name": "magic_distance_z_n_interaction",
            "formula": "exp(-z_dist^2/8) * exp(-n_dist^2/8)",
            "inputs": ["Z", "N"],
            "forbidden_inputs": [
...
def render_report(metrics: dict[str, Any]) -> str:
    ...
    lines.append("")
    lines.append(
        "- Feature: `interaction = exp(-z_dist^2/8) * exp(-n_dist^2/8)` "
    ...
    lines.append("")
    lines.append(
        "- Feature uses only `Z`, `N` (via magic-distance from published "
    ...
    lines.append("")
    lines.append(
        "Per the TASK-0451 specification, this lane uses a custom verdict set "
        "capped at `SHELL_ADJACENT_DIAGNOSTIC`. No registry expansion is "
    ...
    for cv in CUSTOM_VERDICTS:
        meaning = {
            "SHELL_ADJACENT_DIAGNOSTIC": "candidate beat baseline + all four controls and is sign-stable, but the lane caps here (no registry expansion)",
        ...
        "preflight": {
            ...
            "checks": [
                ...
                },
                {
                    ...
                    "status": "PASS",
                    "notes": (
                        "Interaction feature is a closed-form product of magic-distance "
                        "Gaussians; uses only Z and N. No residual or future "
                ...
                },
                {
                    ...
                    "status": "PASS",
                    "notes": (
                        "smooth_a, asymmetry_only, parity_only, matched_degree_random "
                ...
                },
                {
                    ...
                    "notes": (
                        ...
                        "SHELL_ADJACENT_DIAGNOSTIC by custom vocabulary."
                    ),
                },
            ],
        },
...
def render_preflight(metrics: dict[str, Any]) -> str:
    return (
        ...
        "- Four controls run end-to-end: smooth_a + asymmetry_only + parity_only + "
        ...
        "docs/reviews/nuclear-magic-distance-interaction-control-lane.md.\n"
