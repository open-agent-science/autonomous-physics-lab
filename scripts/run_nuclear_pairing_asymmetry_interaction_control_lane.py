"""TASK-0474 nuclear pairing-asymmetry interaction control lane.

Tests whether a single residual-free interaction feature
`r_corr = beta * pairing_sign(Z,N) * ((N-Z)/A)`
fit on the NMD-0002 training slice survives four controls plus a
...
HYP_PROPOSAL_PATH = (
    ...
    "HYP-PROPOSAL-0056-pairing-asymmetry-interaction.yaml"
)
EXP_PROPOSAL_PATH = (
    "experiment_proposals/nuclear-mass/"
    "EXP-PROPOSAL-0022-nuclear-pairing-asymmetry-interaction.yaml"
)
...
DEFAULT_REVIEW_PATH = (
    ...
    / "docs"
    / "reviews"
    / "nuclear-pairing-asymmetry-interaction-control-lane.md"
)

MATCHED_DEGREE_RANDOM_SEED = 474
SURVIVAL_MARGIN_MEV = 0.25
LOO_SIGN_FLIP_THRESHOLD = 2

# Custom verdict vocabulary per TASK-0474 specification
CUSTOM_VERDICTS = (
    "BOUNDED_DIAGNOSTIC",
    "CONTROL_DOMINATED",
...
# --------------------------------------------------------------------------- #
# Residual-free interaction feature
# --------------------------------------------------------------------------- #


def neutron_excess(z: int, n: int, a: int) -> float:
    if a <= 0:
        return 0.0
    return float(n - z) / float(a)


def pairing_only_feature(z: int, n: int, a: int) -> float:
    if a <= 0:
        return 0.0
    return float(pairing_sign(z, n)) / float(np.sqrt(float(a)))


def interaction_feature(z: int, n: int, a: int) -> float:
    """`pairing_sign(Z,N) * ((N-Z)/A)`. Closed-form, residual-free."""
    return float(pairing_sign(z, n)) * neutron_excess(z, n, a)
...
def fit_beta(training_rows: list[dict[str, Any]]) -> float:
    x = np.asarray(
        [
            interaction_feature(int(r["Z"]), int(r["N"]), int(r["A"]))
...
def candidate_correction(rows: list[dict[str, Any]], beta: float) -> list[float]:
    return [
        beta * interaction_feature(int(r["Z"]), int(r["N"]), int(r["A"]))
        for r in rows
    ]
...
def pairing_only_control(
    training_rows: list[dict[str, Any]],
    rows: list[dict[str, Any]],
) -> list[float]:
    train_x = np.asarray(
        [
            pairing_only_feature(int(r["Z"]), int(r["N"]), int(r["A"]))
            for r in training_rows
        ],
        dtype=float,
    )
    ...
    beta = _fit_scalar(train_x, train_y)
    return [
        beta * pairing_only_feature(int(r["Z"]), int(r["N"]), int(r["A"]))
        for r in rows
    ]


def matched_degree_random_control(
    training_rows: list[dict[str, Any]],
    rows: list[dict[str, Any]],
    *,
    seed: int = MATCHED_DEGREE_RANDOM_SEED,
) -> list[float]:
    """Cyclic-shift the interaction feature across training rows, fit beta, apply."""
    ...
    # For evaluation: apply the same beta to the unshuffled true feature on
    # every row. The control's point is that the *training fit* sees a
    # broken Z/N correspondence; evaluation uses the true geometry. If the
    # candidate is real, this should under-perform.
    return [
        beta * interaction_feature(int(r["Z"]), int(r["N"]), int(r["A"]))
        for r in rows
    ]
...
def decide_verdict(metrics: dict[str, Any]) -> tuple[str, list[str]]:
    ...
    controls = {
        ...
        "pairing_only": metrics["control_pairing_only"],
        "matched_degree_random": metrics["control_matched_degree_random"],
    }
    ...
    rationale.append(
        "Candidate beats baseline and all four controls by the survival margin "
...
def build_metrics() -> dict[str, Any]:
    ...
    pairing_full = pairing_only_control(training_rows, full_known_rows)
    matched_full = matched_degree_random_control(training_rows, full_known_rows)
    ...
    training_overview = [
        {
            ...
            "N": int(r["N"]),
            "A": int(r["A"]),
            "pairing_sign": int(pairing_sign(int(r["Z"]), int(r["N"]))),
            "neutron_excess": round(
                neutron_excess(int(r["Z"]), int(r["N"]), int(r["A"])), 6
            ),
            "interaction_feature": round(
                interaction_feature(int(r["Z"]), int(r["N"]), int(r["A"])), 6
            ),
    ...
    metrics: dict[str, Any] = {
        ...
        "feature_definition": {
            "name": "pairing_asymmetry_interaction",
            "formula": "pairing_sign(Z,N) * ((N-Z)/A)",
            "inputs": ["Z", "N", "A"],
            "forbidden_inputs": [
        ...
        "control_pairing_only": split(pairing_full),
        "control_matched_degree_random": split(matched_full),
    ...
    metrics["candidate_vs_pairing_only_full_known_delta_mev"] = _delta(
        metrics["control_pairing_only"]["full_known"]["mae_mev"],
        metrics["candidate"]["full_known"]["mae_mev"],
    )
    metrics["candidate_vs_matched_degree_random_full_known_delta_mev"] = _delta(
        metrics["control_matched_degree_random"]["full_known"]["mae_mev"],
        metrics["candidate"]["full_known"]["mae_mev"],
    )
...
def render_report(metrics: dict[str, Any]) -> str:
    lines: list[str] = []
    lines.append("# Nuclear Pairing-Asymmetry Interaction Control Lane")
    lines.append("")
    ...
    lines.append("")
    lines.append(
        "- Feature: `interaction = pairing_sign(Z,N) * ((N-Z)/A)` "
    ...
    for surface in ("training_lstsq", "primary_holdout", "full_known"):
        ...
        po = metrics["control_pairing_only"][surface]["mae_mev"]
        mr = metrics["control_matched_degree_random"][surface]["mae_mev"]
        lines.append(
    ...
    lines.append("")
    lines.append(
        "- Feature uses only `Z`, `N`, and `A` through pairing sign and "
        "neutron excess. No baseline residual, error rank, candidate-fit "
        "residual, source-status, or future comparison row enters feature "
        "construction. PASS"
    )
    lines.append(
    ...
    lines.append(
        "- All four controls (smooth_a, asymmetry_only, pairing_only, "
    ...
    lines.append("")
    lines.append(
        "Per the TASK-0474 specification, this lane uses a custom verdict set "
        "capped at `BOUNDED_DIAGNOSTIC`. No registry expansion is "
        "authorized regardless of outcome; the Nuclear controls-first gauntlet "
    ...
    for cv in CUSTOM_VERDICTS:
        meaning = {
            "BOUNDED_DIAGNOSTIC": "candidate beat baseline + all four controls and is sign-stable, but the lane caps here (no registry expansion)",
            ...
            "checks": [
                {
                    ...
                    "notes": (
                        ...
                        "feature, four declared controls + coefficient LOO stability, "
                ...
                },
                {
                    ...
                    "status": "PASS",
                    "notes": (
                        "Interaction feature is closed-form pairing sign times "
                        "neutron excess; uses only Z, N, and A. No residual or "
                ...
                },
                {
                    ...
                    "status": "PASS",
                    "notes": (
                        "smooth_a, asymmetry_only, pairing_only, matched_degree_random "
                        "controls all run end-to-end."
                    ),
                },
                {
...
def render_preflight(metrics: dict[str, Any]) -> str:
    return (
        ...
        "- Hypothesis family: fresh pairing-asymmetry interaction under the "
        "controls-first gauntlet.\n"
        "- Allowed inputs: Z, N, A, pairing sign, and neutron excess, all "
        "deterministic residual-free functions of committed row identifiers.\n"
        ...
        "- Four controls run end-to-end: smooth_a + asymmetry_only + pairing_only + "
        "matched_degree_random.\n"
        "- Coefficient stability LOO reported.\n"
        "- Failure condition declared before scoring: candidate must beat baseline by "
        "≥ 0.25 MeV on full_known AND beat all four controls by the same margin AND "
        ...
        "docs/reviews/nuclear-pairing-asymmetry-interaction-control-lane.md.\n"
        "- Custom verdict vocabulary caps at BOUNDED_DIAGNOSTIC; no "
        "registry expansion or PRED entry is authorized.\n"
        "- Wording boundary: forbidden terms include discovery, new nuclear law, "
        "broad formula, nuclear-law wording, or breakthrough framing.\n"
    )
...
def render_review_summary(metrics: dict[str, Any]) -> str:
    return (
        ...
        f"- Full-known candidate vs pairing_only: "
        f"{_fmt(metrics['candidate_vs_pairing_only_full_known_delta_mev'])} MeV.\n"
        f"- Full-known candidate vs matched_degree_random: "
        f"{_fmt(metrics['candidate_vs_matched_degree_random_full_known_delta_mev'])} MeV.\n"
