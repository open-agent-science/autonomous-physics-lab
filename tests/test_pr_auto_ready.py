...
def test_auto_ready_workflow_is_opt_in_and_uses_trusted_finish_gate() -> None:
    workflow = yaml.safe_load(AUTO_READY_WORKFLOW.read_text(encoding="utf-8"))
    ...
    assert workflow["jobs"]["auto-ready"]["runs-on"] == "ubuntu-latest"

    steps = workflow["jobs"]["auto-ready"]["steps"]
    checkout = steps[0]
    finish_gate = steps[-1]
