"""Tests for the TASK-0451 magic-distance Z×N interaction control lane runner."""
...
from scripts.run_nuclear_magic_distance_interaction_control_lane import (  # noqa: E402
...
def test_interaction_feature_peaks_at_double_magic() -> None:
    """Interaction = 1.0 exactly at double-magic; decays away from it."""
...
def test_interaction_feature_factorizes() -> None:
    """interaction(Z, N) == gauss(z_dist) * gauss(n_dist)."""
    for z, n in [(60, 100), (10, 11), (50, 82), (40, 90)]:
        gz = math.exp(-magic_distance(z) ** 2 / GAUSSIAN_TWO_SIGMA_SQUARED)
        gn = math.exp(-magic_distance(n) ** 2 / GAUSSIAN_TWO_SIGMA_SQUARED)
        assert interaction_feature(z, n) == pytest.approx(gz * gn, abs=1e-12)
...
def test_decide_verdict_shell_adjacent_when_full_gauntlet_passed() -> None:
    ...
    verdict, rationale = decide_verdict(metrics)
    assert verdict == "SHELL_ADJACENT_DIAGNOSTIC"
...
def test_runner_smoke_against_committed_data() -> None:
    ...
    assert metrics["agent_run_id"] == "AGENT-RUN-0045"
...
def test_committed_metrics_matches_runner_output() -> None:
    committed = json.loads(
        (REPO_ROOT / "agent_runs" / "AGENT-RUN-0045" / "metrics.json").read_text(
            encoding="utf-8"
        )
    )
