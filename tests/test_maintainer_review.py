...
def test_build_review_report_generated_board_sync_closeout_pr_is_merge_ok(
    tmp_path: Path,
) -> None:
    branch = "agent/github-actions/bot/closeout-board-sync-28334921926"
    ...
    body = (
        _full_pr_body(
            ...
            primary_reference=(
                ...
                "- Closed Task Files: generated task navigation after `5946fd27`"
            ),
        )
        .replace("- Contributor ID: roman", "- Contributor ID: github-actions")
        .replace("- GitHub username: gladunrv", "- GitHub username: github-actions[bot]")
        .replace("- Agent tool: codex", "- Agent tool: GitHub Actions")
    )
    pr_metadata = PullRequestMetadata(
        number=1292,
    ...
    with (
        ...
        report = build_review_report(tmp_path, pull_request=1292)

    assert report.task_id == "TASK-CLOSEOUT"
    assert report.verdict == "MERGE_OK"
    assert report.blockers == ()
    assert any(
        "Generated board-sync closeout PR updates only docs/task-views/*.md" in item
